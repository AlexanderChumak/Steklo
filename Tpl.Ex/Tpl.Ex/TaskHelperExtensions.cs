...
namespace Tpl.Ex
{
    ...
    {
        private static Task<AsyncVoid> _defaultCompleted = TaskHelper.FromResult<AsyncVoid>(default(AsyncVoid));
        ...
        // </summary>
        public static Task Catch(this Task task, Func<CatchInfo, CatchInfo.CatchResult> continuation, CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        // block, so returning a real async task should never be done from Catch().
        // </summary>
        public static Task<TResult> Catch<TResult>(this Task<TResult> task, Func<CatchInfo<TResult>, CatchInfo<TResult>.CatchResult> continuation, CancellationToken cancellationToken = default(CancellationToken))
        {
            // Fast path for successful tasks, to prevent an extra TCS allocation
            if (task.Status == TaskStatus.RanToCompletion)
            {
        ...
        [SuppressMessage("Microsoft.Naming", "CA2204:Literals should be spelled correctly", MessageId = "TaskHelperExtensions", Justification = "This is the name of a class.")]
                ...
                if (task.IsFaulted)
                {
                    try
                    {
                        ...
                        if (resultTask == null)
                        {
                            ...
                            throw new InvalidOperationException("You must set the Task property of the CatchInfo returned from the TaskHelperExtensions.Catch continuation.");
                        }
                    ...
                    {
                        return TaskHelper.FromError<TResult>(ex);
                    }
                }
                if (task.IsCanceled || cancellationToken.IsCancellationRequested)
                {
                    return TaskHelper.Canceled<TResult>();
                }
                ...
                if (syncContext != null)
                {
                    syncContext.Post(state =>
                    {
                        try
                        {
                            ...
                            if (resultTask == null)
                            {
                                throw new InvalidOperationException("You cannot return null from the TaskHelperExtensions.Catch continuation. You must return a valid task or throw an exception.");
                            }
            ...
            if (task.IsCompleted)
            {
                switch (task.Status)
                {
                    ...
                    case TaskStatus.Faulted:
                        TaskHelper.TrySetFromTask(tcs, task);
                        break;
            ...
            where TTask : Task
        {
            return task.ContinueWith(innerTask =>
            {
                switch (innerTask.Status)
                {
                    case TaskStatus.Canceled:
                    case TaskStatus.Faulted:
                        TaskHelper.TrySetFromTask(tcs, innerTask);
                        break;
        ...
        internal static Task<object> CastToObject(this Task task)
        {
            ...
            if (task.IsCompleted)
            {
                if (task.IsFaulted)
                {
                    return TaskHelper.FromErrors<object>(task.Exception.InnerExceptions);
                }
                if (task.IsCanceled)
                {
                ...
                if (task.Status == TaskStatus.RanToCompletion)
                {
                    return TaskHelper.FromResult<object>((object)null);
                }
            }
        ...
        internal static Task<object> CastToObject<T>(this Task<T> task)
        {
            ...
            if (task.IsCompleted)
            {
                ...
                if (task.Status == TaskStatus.RanToCompletion)
                {
                    return TaskHelper.FromResult<object>((object)task.Result);
                }
            }
        ...
        internal static Task<TOuterResult> CastFromObject<TOuterResult>(this Task<object> task)
        {
            ...
            if (task.IsCompleted)
            {
                if (task.IsFaulted)
                {
                    return TaskHelper.FromErrors<TOuterResult>(task.Exception.InnerExceptions);
                }
                if (task.IsCanceled)
                {
                    return TaskHelper.Canceled<TOuterResult>();
                }
                if (task.Status == TaskStatus.RanToCompletion)
                {
                    try
                    {
                        return TaskHelper.FromResult<TOuterResult>((TOuterResult)task.Result);
                    }
                    catch (Exception exception)
                    {
                        return TaskHelper.FromError<TOuterResult>(exception);
                    }
                }
            }
        ...
        private static Action<Task> GetRethrowWithNoStackLossDelegate()
        {
            ...
            if (getAwaiterMethod != null)
            {
                ...
                var taskParameter = System.Linq.Expressions.Expression.Parameter(typeof(Task));
        ...
        // </summary>
        public static Task Then(this Task task, Action continuation, CancellationToken cancellationToken = default(CancellationToken), bool runSynchronously = false)
        {
        ...
        // to completion (i.e., was not cancelled and did not fault).
        // </summary>
        public static Task<TOuterResult> Then<TOuterResult>(this Task task, Func<TOuterResult> continuation, CancellationToken cancellationToken = default(CancellationToken), bool runSynchronously = false)
        {
            return task.ThenImpl(t => TaskHelper.FromResult(continuation()), cancellationToken, runSynchronously);
        }
        ...
        // to completion (i.e., was not cancelled and did not fault).
        // </summary>
        public static Task Then(this Task task, Func<Task> continuation, CancellationToken cancellationToken = default(CancellationToken), bool runSynchronously = false)
        {
        ...
        // to completion (i.e., was not cancelled and did not fault).
        // </summary>
        public static Task<TOuterResult> Then<TOuterResult>(this Task task, Func<Task<TOuterResult>> continuation, CancellationToken cancellationToken = default(CancellationToken), bool runSynchronously = false)
        {
        ...
        // </summary>
        [SuppressMessage("Microsoft.Web.FxCop", "MW1201:DoNotCallProblematicMethodsOnTask", Justification = "The usages here are deemed safe, and provide the implementations that this rule relies upon.")]
        public static Task Then<TInnerResult>(this Task<TInnerResult> task, Action<TInnerResult> continuation, CancellationToken cancellationToken = default(CancellationToken), bool runSynchronously = false)
        {
        ...
        {
            return task.ThenImpl(t => TaskHelper.FromResult(continuation(t.Result)), cancellationToken, runSynchronously);
        }
        ...
        // </summary>
        [SuppressMessage("Microsoft.Web.FxCop", "MW1201:DoNotCallProblematicMethodsOnTask", Justification = "The usages here are deemed safe, and provide the implementations that this rule relies upon.")]
        public static Task Then<TInnerResult>(this Task<TInnerResult> task, Func<TInnerResult, Task> continuation, CancellationToken token = default(CancellationToken), bool runSynchronously = false)
        {
            ...
            where TTask : Task
        {
            ...
            if (task.IsCompleted)
            {
                ...
                if (task.Status == TaskStatus.RanToCompletion)
                {
                    ...
                    {
                        return TaskHelper.FromError<TOuterResult>(ex);
                    }
                }
            }
        ...
        {
            return TaskHelper.RunSynchronously<AsyncVoid>(() =>
            {
                action();
        ...
        internal static Task<TResult> ToTask<TResult>(this Task task, CancellationToken cancellationToken = default(CancellationToken), TResult result = default(TResult))
        {
            ...
            if (task.IsCompleted)
            {
                if (task.IsFaulted)
                {
                    return TaskHelper.FromErrors<TResult>(task.Exception.InnerExceptions);
                }
                if (task.IsCanceled || cancellationToken.IsCancellationRequested)
                {
                    return TaskHelper.Canceled<TResult>();
